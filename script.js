        ...
        fetch(`/api/timezone?tz=${encodeURIComponent(timezone)}`)
            ...
            .then(data => {
                if (!data.error) {
                    ...
                    visitorTime.style.opacity = '0';
                    myTime.style.opacity = '0';
                    
                    setTimeout(() => {
                        visitorTime.textContent = data.visitor_time;
                        myTime.textContent = data.marcio_time;
                        
                        visitorTime.style.opacity = '1';
                        myTime.style.opacity = '1';
                    }, 300);
                }
            })
            .catch(console.error);
...
function initTimeDisplay() {
    function updateTime() {
        // Fallback para caso a API não responda
        const fallbackTime = () => {
            const now = new Date();
            document.getElementById('visitor-time').textContent = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        ...
        // Tenta detectar o fuso horário do visitante
        try {
            const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
            ...
            fetch(`/api/timezone?tz=${encodeURIComponent(timezone)}`)
                ...
                .catch(() => fallbackTime()); // Se falhar, usa o fallback
        } catch (e) {
            fallbackTime();
        }
    }
...
function initTimeDisplay() {
    function updateClockStyles() {
      ...
      timeDisplays.forEach(display => {
        display.style.color = isDark ? '#34d399' : '#3b82f6'; // Cores diferentes para cada tema
    ...
    function updateTime() {
      ...
      updateClockStyles(); // Aplica os estilos
    }
